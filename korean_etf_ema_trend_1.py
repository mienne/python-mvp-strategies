...
class KoreanETFARPStrategy:
    ...
    def __init__(self, etf_list, lookback_ema=112, lookback_corr=250, 
                 target_vol=0.15, max_loss=-0.15, rebalance_freq='W'):
        ...
        for ticker in self.etf_list:
            try:
                if ticker in self.etf_mapping:
                    ...
                    if len(df) > 0:
                        data[ticker] = df
                        print(f"Loaded {ticker} ({code}): {len(df)} rows")
                else:
        ...
        for date in corr_matrix.index.get_level_values(0).unique():
            if date in returns.index:
                try:
                    ...
                    inv_corr = np.linalg.inv(daily_corr + 0.01 * np.eye(len(daily_corr)))
        ...
        for ticker in positions.columns:
            for date in positions.index:
                if ticker in self.current_positions:
                    entry_price = self.entry_prices.get(ticker, prices.loc[date, ticker])
                    ...
                    if pnl < self.max_loss:
                        ...
                        print(f"{date}: {ticker} 손절 실행 (손실: {pnl:.2%})")
    ...
    def backtest(self, start_date, end_date):
        ...
        prices, returns = self.fetch_data(
            start_date - timedelta(days=self.lookback_corr*2), 
            end_date
        )
        ...
        if self.rebalance_freq == 'W':
            positions = positions.resample('W-FRI').last().reindex(positions.index).fillna(method='ffill')
    ...
    def visualize_results(self, results, prices):
        ...
        if 'KODEX 200' in prices.columns:
            ...
            benchmark_cumulative = benchmark_cumulative.reindex(cumulative_returns.index)
            benchmark_cumulative.plot(ax=ax1, label='KODEX 200', linewidth=2, color='gray', alpha=0.7)
        ...
        ax6.bar(range(len(current_signals)), current_signals.values, color=colors, alpha=0.7)
        ...
        plt.suptitle('Korean ETF ARP-EMA Strategy Backtest Results', fontsize=16, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_correlation_heatmap(self, returns):
        ...
        sns.heatmap(corr_recent, annot=True, fmt='.2f', cmap='coolwarm', 
                    center=0, ax=ax2, square=True)
        ax2.set_title('Recent 60 Days Correlation Matrix', fontsize=14, fontweight='bold')
    ...
    def plot_risk_return_scatter(self, returns, positions):
        ...
        # 산점도
        scatter = ax.scatter(annual_vols * 100, annual_returns * 100, 
                           s=current_positions * 1000, alpha=0.6, 
                           c=annual_returns, cmap='RdYlGn')
        
        # ETF 이름 표시
        for etf in returns.columns:
            ax.annotate(etf, (annual_vols[etf] * 100, annual_returns[etf] * 100),
                       xytext=(5, 5), textcoords='offset points', fontsize=8)
        
        # 효율적 프론티어 근사선
        ax.plot([0, annual_vols.max() * 100], [0, annual_vols.max() * 100 * 0.5], 
                'k--', alpha=0.5, label='0.5 Sharpe Ratio')
    ...
    def visualize_results_separate(self, results, prices, save_path='output'):
        """백테스트 결과를 개별 그래프로 시각화하여 저장"""
        import os
        ...
        ax1.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f'{save_path}/1_cumulative_returns.png', dpi=150, bbox_inches='tight')
        plt.close()
        ...
        ax3.set_ylabel('Month')
        plt.tight_layout()
        plt.savefig(f'{save_path}/3_monthly_returns_heatmap.png', dpi=150, bbox_inches='tight')
        plt.close()
        ...
        ax7.set_title('Performance Metrics', fontsize=14, fontweight='bold', pad=20)
        plt.tight_layout()
        plt.savefig(f'{save_path}/7_performance_metrics.png', dpi=150, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    # 1. 개별 그래프로 시각화 저장
    strategy.visualize_results_separate(results, prices, save_path='output')
    
    # 2. 상관관계 히트맵
    fig_corr = strategy.plot_correlation_heatmap(returns)
    plt.savefig('output/8_correlation_heatmap.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    # 3. 리스크-수익률 산점도
    fig_scatter = strategy.plot_risk_return_scatter(returns, results['positions'])
    plt.savefig('output/9_risk_return_scatter.png', dpi=150, bbox_inches='tight')
    plt.close()
