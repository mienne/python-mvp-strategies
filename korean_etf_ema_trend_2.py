...
def calculate_ema_signals(etf_data, ema_period=112, lookback=250):
   ...
   for etf, df in etf_data.items():
       ...
       rolling_mean = df["Returns"].rolling(window=lookback, min_periods=lookback//2).mean()
       rolling_std = df["Returns"].rolling(window=lookback, min_periods=lookback//2).std()
       df["Normalized_Returns"] = (df["Returns"] - rolling_mean) / (rolling_std + 1e-6)
       
       # EMA 계산
       df[f"EMA_{ema_period}"] = df["Normalized_Returns"].ewm(span=ema_period, adjust=False).mean()
...
def calculate_position_sizes(signals_dict, correlation_weights, target_vol=0.15, num_tranches=5):
   """포지션 크기 계산 (분할 진입 고려)"""
   position_sizes = {}
   ...
   for date in all_dates:
       ...
       for etf in signals_dict.keys():
           if date in signals_dict[etf].index:
               ...
               # 상관관계 가중치 적용
               adjusted_strength = strength * corr_weights.get(etf, 1/len(signals_dict))
       ...
       if total_signal_strength > 0:
           for etf, signal_info in etf_signals.items():
               # 기본 포지션 크기
               base_position = (signal_info["strength"] / total_signal_strength) * signal_info["signal"]
       ...
       if date in position_sizes and position_sizes[date]:
           for etf, target_info in position_sizes[date].items():
               if is_rebalance_day:
                   ...
                   # 분할 진입/청산 로직
                   if abs(target_pos - current_pos) > target_info["tranche_size"] * 0.5:
                       # 한 트랜치씩 조정
       ...
       if i == 0:
           ...
           for etf in managed_positions.columns:
               ...
               if date in etf_data[etf].index and prev_date in etf_data[etf].index:
                   # 수익률 계산
                   price_change = (etf_data[etf].loc[date, "Close"] / 
                                  etf_data[etf].loc[prev_date, "Close"] - 1)
                   ...
                   if etf in entry_prices and prev_pos != 0:
                       ...
                       position_pnl = (current_price / entry_price - 1) * np.sign(prev_pos)
...
class KoreanETFIntegratedExecution:
   ...
   def run_complete_analysis(self, data_path, start_date, end_date, output_path=None):
       ...
       print(f"Strategy Config: {self.config.ema_period}-day EMA, {self.config.target_vol:.0%} target vol")
       ...
       # 실행 단계
       execution_steps = [
           ("Validate Environment", lambda: self.validate_execution_environment(data_path)),
           ("Load Data", lambda: self.execute_data_loading_phase(data_path, start_date, end_date)),
           ...
           ("Calculate Performance", lambda: self.execute_performance_analysis_phase()),
   ...
   def _calculate_risk_metrics(self):
       ...
       risk_metrics = {
           ...
           "tracking_error": (returns - benchmark_returns).std() * np.sqrt(self.config.trading_days_per_year),
           "information_ratio": (returns.mean() - benchmark_returns.mean()) / (returns - benchmark_returns).std(),
           "downside_deviation": returns[returns < 0].std() * np.sqrt(self.config.trading_days_per_year),
   ...
   def _calculate_attribution_analysis(self):
       ...
       for etf in self.config.etf_list:
           if etf in self.results["etf_data"] and etf in self.results["managed_positions"].columns:
               ...
               if len(common_dates) > 0:
                   ...
                   aligned_positions = etf_positions.reindex(common_dates, fill_value=0)
                   
                   etf_contribution = (aligned_positions.shift(1) * aligned_returns).sum()
   ...
   def _plot_etf_signals(self, output_path):
       ...
       for idx, etf in enumerate(top_etfs):
           if etf in self.results["etf_data"]:
               ...
               # 가격
               ax.plot(data.index, data["Close"], label=f"{self.config.etf_names.get(etf, etf)} Price", alpha=0.7)
               ...
               ax.fill_between(data.index, data["Close"].min(), data["Close"].max(),
                              where=long_periods, alpha=0.2, color='green', label='Long')
               ax.fill_between(data.index, data["Close"].min(), data["Close"].max(),
                              where=short_periods, alpha=0.2, color='red', label='Short')
       ...
       plt.tight_layout()
       plt.savefig(f"{output_path}/etf_signals_analysis.png", dpi=300, bbox_inches="tight")
       plt.close()
   ...
   def _plot_position_heatmap(self, output_path):
       ...
       sns.heatmap(renamed_positions.T, cmap='RdBu_r', center=0, 
                  annot=True, fmt='.2f', cbar_kws={'label': 'Position Size'})
   ...
   def _plot_correlation_evolution(self, output_path):
       ...
       for etf in top_etfs:
           if etf in corr_weights.columns:
               ax.plot(corr_weights.index, corr_weights[etf], 
                      label=self.config.etf_names.get(etf, etf), alpha=0.8)
       ...
       plt.tight_layout()
       plt.savefig(f"{output_path}/correlation_weights.png", dpi=300, bbox_inches="tight")
       plt.close()
   ...
   def _plot_performance_attribution(self, output_path):
       ...
       plt.tight_layout()
       plt.savefig(f"{output_path}/performance_attribution.png", dpi=300, bbox_inches="tight")
       plt.close()
   
   def _save_detailed_results(self, output_path):
       try:
           # 수익률 저장
           self.results["portfolio_returns"].to_csv(f"{output_path}/portfolio_returns.csv")
           
           # 포지션 저장
           self.results["managed_positions"].to_csv(f"{output_path}/managed_positions.csv")
           
           # 거래 내역 저장
           if not self.results["trade_log"].empty:
               self.results["trade_log"].to_csv(f"{output_path}/trade_log.csv", index=False)
           
           # 성과 통계 저장
           self.results["performance_stats"].to_csv(f"{output_path}/performance_stats.csv")
           ...
           # 실행 로그 저장
           pd.DataFrame(self.execution_log).to_csv(f"{output_path}/execution_log.csv", index=False)
   ...
   def _print_results_summary(self):
       ...
       if "performance_stats" in self.results:
           strategy_stats = self.results["performance_stats"].loc["Korean ETF Strategy"]
...
def run_korean_etf_tests():
   ...
   try:
       ...
       etf_data = load_korean_etf_data(temp_dir, ["069500"], "2020-01-01", "2020-12-31")
   ...
   try:
       test_returns = {
           "ETF1": pd.Series(np.random.randn(300), index=pd.date_range("2020-01-01", periods=300)),
           "ETF2": pd.Series(np.random.randn(300), index=pd.date_range("2020-01-01", periods=300)),
       }
...
if __name__ == "__main__":
   # 데이터 경로 설정
   DATA_PATH = "/Users/enne/Documents/dev/python-etf-strategy/data"
